            ...
            case 'GET':
                ...
                case 'PUT':
                    if ($_header == $_token_put) {
                        ...
                        $control = new Controlador();
                        $body = json_decode(file_get_contents("php://input", true));
                        if($body->nombre && !$body->texto && !$body->texto_adicional){
                            $respuesta = $control->putNombreById($body->nombre, $body->id);
                            http_response_code(200);
                            echo json_encode(["data" => $respuesta]);
                        }else if($body->texto && !$body->texto_adicional && !$body->nombre){
                            $respuesta = $control->putTextoById($body->texto, $body->id);
                            http_response_code(200);
                            echo json_encode(["data" => $respuesta]);
                        }else if($body->texto_adicional && !$body->nombre && !$body->texto){
                            $respuesta = $control->putTextoAdicionalById($body->texto_adicional, $body->id);
                            http_response_code(200);
                            echo json_encode(["data" => $respuesta]);
                        }else if($body->texto && $body->nombre && $body->texto_adicional){
                            $respuesta = $control->putAll($body->nombre, $body->texto , $body->texto_adicional, $body->id);
                            http_response_code(200);
                            echo json_encode(["data" => $respuesta]);
                        }else{
